...
public class Torus {
    ...
    */
    ...
    public Torus() {
        //(Radio mayor, radio menor, Segmentos Mayores, Segmentos Menores)
        torusCoords = createCoordsTorus(1f, 0.30f, 32, 16);
    ...
    */
    ...
    // CLASE Torus.java - Método createCoordsTorus modificado para orientación horizontal
    ...
    private float[] createCoordsTorus(float R, float r, int mayor, int menor) {
        ...
        for (int i = 0; i < mayor; i++) {
            ...
            for (int j = 0; j < menor; j++) {
                ...
                // Antes: X = (R + r * cosP) * cosT
                v[p++] = (R + r * cosP) * cosT;

                // INTERCAMBIO CLAVE: Y se convierte en Z, Z se convierte en Y

                // Nueva Y: El componente que define la altura del círculo pequeño (r * sinP)
                v[p++] = r * sinP;

                // Nueva Z: El componente que antes definía la altura del toroide
                v[p++] = (R + r * cosP) * sinT;
            }
        }
        return v;
