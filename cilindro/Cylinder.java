...
public class Cylinder {
    ...
    float color[] = {1f, 0f, 0f, 1f}; //Se define el color del cilindro en RGBA


    
    //Vertex Shader se encarga de la geometría y la posición. Se ejecuta una vez por cada punto (vértice) del cilindro.
    private final String vertexShaderCode =
            "uniform mat4 uMVPMatrix;" +  //matriz de transformación. Le dice al programa cómo mover, rotar y escalar el cilindro en el espacio 3D
                    "attribute vec4 vPosition;" +  //Es la posición original del punto (X, Y, Z, y W). Esta es la coordenada que le pasamos desde el FloatBuffer
                    "void main(){ gl_Position = uMVPMatrix * vPosition; }";  //Toma la posición original del punto (vPosition), le aplica todas las transformaciones de movimiento/rotación (uMVPMatrix), y el resultado se guarda en gl_Position
    ...
    public void draw(float[] mvpMatrix) {
        ...
        GLES20.glUseProgram(mProgram); //Activa el programa

        positionHandle = GLES20.glGetAttribLocation(mProgram, "vPosition"); //Buscar ubicación (posición)
        colorHandle = GLES20.glGetUniformLocation(mProgram, "vColor"); //Buscar ubicación (color)
        matrixHandle = GLES20.glGetUniformLocation(mProgram, "uMVPMatrix"); //Buscar ubicación (matriz)

        //glUniformMatrix4fv(Indica donde va la matriz, una matriz, no transpuesta, array datos de la matriz 16, leer desde 0)
        GLES20.glUniformMatrix4fv(matrixHandle, 1, false, mvpMatrix, 0); //Envía la matriz de movimiento, rotación y escala 

        //glUniform4fv(Indica donde va el color, 1 vector de color, array de 4 números float que contienen los valores RGBA, leer desde 0)
        GLES20.glUniform4fv(colorHandle, 1, color, 0); //Enviar el Color -Envía el array de color
        
        //glEnableVertexAttribArray(canal de datos de posición (X, Y, Z) para que OpenGL pueda recibir y procesar las coordenadas)
        GLES20.glEnableVertexAttribArray(positionHandle); //Activar la posición.

        // ---------------- TAPAS -----------------
        //glVertexAttribPointer(coordenadas de posición, tamaño 3 por x,y,x , coordenadas de tipo FLOAT, sin normalizar, 12 bytes, contiene las coordenadas)
        GLES20.glVertexAttribPointer(positionHandle, 3, GLES20.GL_FLOAT, false, STRIDE, bottomBuffer);//lee como se formará la base

        //glDrawArrays(GLES20.GL_TRIANGLE_FAN, inicia en 0, cantidad de puntos base)
        //GL_TRIANGLE_FAN - comparten vertice de trignulos en la base   I><I
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_FAN, 0, bottomCount); //Dibuja e interpreta
        
        //mismo que en bottomBuffer, cambia el glVertexAttribPointer(........, topBuffer = contiene las coordenadas)
        GLES20.glVertexAttribPointer(positionHandle, 3, GLES20.GL_FLOAT, false, STRIDE, topBuffer);
        
        //glDrawArrays(GLES20.GL_TRIANGLE_FAN, inicia en 0, cantidad de puntos tapa)
        //mismo que bottom - GL_TRIANGLE_FAN - comparten vertice de trignulos en la tapa   I><I
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_FAN, 0, topCount);

        // ---------------- LATERAL ----------------
        //mismo que en bottom y top - cambia el glVertexAttribPointer(........, sideBuffer = contiene las coordenadas)
        GLES20.glVertexAttribPointer(positionHandle, 3, GLES20.GL_FLOAT, false, STRIDE, sideBuffer);

        //glDrawArrays(GLES20.GL_TRIANGLE_FAN, inicia en 0, cantidad de puntos laterales)
        //mismo que bottom y top - GL_TRIANGLE_FAN - comparten vertice de trignulos en el lateral   I><I
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, sideCount);

        GLES20.glDisableVertexAttribArray(positionHandle); //Desactivar la posición.
    }
